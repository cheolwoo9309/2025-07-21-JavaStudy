...
public class 비교연산자 {
	...
	public static void main(String[] args) {
		...
		double f1=10.5;
		double d1=10.5;
		boolean result6=(f1==d1);
		
		boolean b1=true;
		boolean b2=false;
		boolean result7=(b1==b2); // false
		boolean result8=(b1!=b2); // true
		...
		boolean result9=(s1==s2); // false 
		boolean result10=(s1!=s2); // true
		boolean result11=(s2==s3); // true
		boolean result12=(s2!=s3); // false
		boolean result13=(s1.equals(s2)); // true
		//                   ------- 실제 저장된 문자열만 비교
		...
		boolean result14=(ss1.equals(ss2)); // false
		boolean result15=(!ss1.equals(ss2)); // true
		// 로그인처리 => id / pw 찾기
		// id중복체크
		// 대소문자 구분 => 비교
		boolean result16=(ss1.equalsIgnoreCase(ss2)); // true
		// 대소문자 구분하지 않고 비교
		// 검색 => AWS 
		 ...
		 *   contains() => 단어 포함 => 검색
		 *   ----------------------------
		 *   stratsWith() => 시작문자열이 같은 경우
		 *   endsWith() => 파일 확장자 (.jpg / .png ...)
		 *   ---------------------------- 서제스트
		 *   
		 *   System / String / Math ...
		 */
