 ...
 *        }
 *     2) 인스턴스 변수
 *        선언위치 : 클래스 내부 블록에 선언 => 클래스 상단이나 하단, 중간에 선언이 가능하다
 *        저장위치 : 객체 생성시에 메모리에 저장 => new를 사용할때 메모리에 저장
 *                                       ---- 새로운 메모리가 생성
 ...
 *         -------- 객체.변수
 *         정적변수 : 클래스변수, 공유변수 라고도 부른다
 *                 ------- 
 *                 클래스명.변수
 *                 => 모든 객체가 데이터를 공유 => static
 *                 => 데이터 묶음, 오라클에 저장된 데이터
 *     *** 초기화 여부
 *         지역변수 : 필수, 직접 대입, 연산결과, 기본값은 없다
 *         인스턴스변수 : 선택, 선언시 대입, 생성자에서 대입, 기본값이 있다
 *         정적변수 : 선택, 선언시 대입, 초기화 블록, 기본값이 있다
 *     *** 초기화 방법
 *         1) 선언과 동시에 초기화 : 명시적 초기화 => 고정 (값이 한가지)
 *         2) 생성자를 통해서 초기화 => 여러가지 값을 지정가능
 ...
 *          | 변수에 대한 초기값을 변경 => 매개변수
 *         ** 모든 클래스는 생성자가 한개 이상이 반드시 필요하다
 *            => 생성자가 없는 경우는 컴파일러가 자동으로 기본 생성자 추가
 *            => 기본 생성자 => 매개변수가 없는 생성자
 *      => 생성자의 종류
 *         기본생성자(디폴트 생성자) => 자동 추가가 될 수 있다
 *           => 시작과 동시에 처리해야 되는 기능 설정할때
 *         매개변수가 있는 생성자
 *           => 객체마다 다른 데이터를 저장하고 싶을때
 *      => 변수
 *         기본형 = int a, double d ..
 *         ------------------------- 기본형 변수 (실제 값)
 *         배열 : 같은 데이터형
